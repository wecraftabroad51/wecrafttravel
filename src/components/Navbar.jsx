...
const MENU = [
  ...
  {
    label: 'รถเช่า', labelEn: 'Car Rental',
    key: 'car-rental', tourType: null,
    mega: [
      { continent: '🚗 รถเช่า', continentEn: '🚗 Car Rental', countries: ['รถเช่าในประเทศ','รถเช่าต่างประเทศ','รับ-ส่งสนามบิน'] },
    ],
  },
  {
    label: 'บริการเสริม', labelEn: 'Extra Services',
    key: 'visa', tourType: null,
    mega: [
            ...
            <div key={g.id} style={{ gridColumn: cols === 2 ? 'span 2' : 'span 1', marginBottom: 14 }}>
              ...
              <button
                onClick={() => {
                  navigate('tours', null, {
                    ...
                    countries: g.countries.map(c => c.code),
                  });
                  onClose();
                }}
                style={{
              ...
              <div style={{ display: 'grid', gridTemplateColumns: `repeat(${cols}, 1fr)`, gap: '1px 14px' }}>
                {g.countries.map(c => (
                  <button key={c.code}
                    onClick={() => {
                      navigate('tours', null, { country: c.code });
                      onClose();
                    }}
                    style={{
